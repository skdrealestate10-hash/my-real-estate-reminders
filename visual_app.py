...
try:
    ...
    st.error("Secrets not found! Please add GMAIL_USER and GMAIL_PASSWORD to Streamlit Secrets.")
    st.stop()
...
if st.session_state.page == "dashboard":
    ...
    for i, row in df[::-1].iterrows():
        with st.container():
            ...
            with col_tx:
                tag_style = "tag-active" if str(row['Status']) == 'Active' else "tag-sent"
                st.markdown(f'<span class="{tag_style}">{str(row["Status"]).upper()}</span> **{row["Task"]}**', unsafe_allow_html=True)
